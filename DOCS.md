...
The code is **generated by crocle** as `crocle-<random>` and passed to the
container via the `CROC_SECRET` environment variable (so it never appears in
the container's process list), instead of letting croc generate one. It is
therefore known as soon as the job starts and is exposed as `code` (plus
`url`) without any log parsing. Status is `waiting` while the container runs;
the container is created with `AutoRemove`, so it is deleted by Docker when
the recipient finishes and the job drops out of the list.
...
- A custom croc code maps directly to one of the three public relays with no
  probing or fallback (unlike auto-generated codes), so a down relay breaks
  that transfer.
